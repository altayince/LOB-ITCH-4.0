...
#define BUILD_BOOK 1
 ...
 start = std::chrono::steady_clock::now();
        ...
        assert(uint64_t(pkt.oid) <
          ...
          //std::cout << "ADD " << (int) pkt.oid << "\t" << pkt.stock_locate << "\t" << (int) pkt.buy << "\t" << (int)pkt.qty << "\t" << (int) pkt.price << "\t" << (long) pkt.timestamp << std::endl;
            order_book::add_order(order_id_t(pkt.oid), book_id_t(pkt.stock_locate),
                                  mksigned(pkt.price, pkt.buy), pkt.qty,int(pkt.buy));
            ++npkts;
      ...
      case (itch_t::ADD_ORDER_MPID): {
        ...
        order_book::add_order(
            ...
            mksigned(pkt.add_msg.price, pkt.add_msg.buy), pkt.add_msg.qty,int(pkt.add_msg.buy));
      ...
      case (itch_t::EXECUTE_ORDER): {
        ...
        auto const pkt = PROCESS<itch_t::EXECUTE_ORDER>::read_from(&buf);
              ...
              order_book::execute_order(order_id_t(pkt.oid), pkt.qty,book_id_t (pkt.stock_locate )  ,  int(pkt.buy) );
              ++npkts;
      ...
      case (itch_t::DELETE_ORDER): {
         ...
         //std::cout << "DELETE " << (int) pkt.oid << "\t" << pkt.stock_locate << "\t" << (int) pkt.buy << "\t" << (long) pkt.timestamp << std::endl;

    
#if BUILD_BOOK


              order_book::delete_order(order_id_t(pkt.oid),book_id_t(pkt.stock_locate),int(pkt.buy));
              ++npkts;
