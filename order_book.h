...
{
    /*
     * This is a hashp map to keep track of our orders. sparse_hash
     * is downloaded from internet and then included into this project.
     * From empirical observation I can conclude that it works faster
     * than boost::unordered_map in our case. You may want to read spp.h
     * for any further information.
     *
     * Itch 4.0 does not generate unique order ID so I wanted to generate
     * a unique number from order ID, book ID and side. Using the unique
     * number I generate, I insert them to a hash map.
     * (long)order_id<<40) + ((int)book_idx << 8) + side is the code piece
     * to generate a unique number from order ID, book ID and side.
     * It's actually no more than concatenating the diverse parts of
     * our numbers. Simple yet effective.
     */
 public:
    spp::sparse_hash_map<long,order> x;
    //boost::unordered_map<long,order> x;

    void reserve()
    {
        x.reserve(1000000);
    }
    T *get(order_id_t(order_id),book_id_t(book_idx),int side)
        {
            //bitwise art arda yazma yolu ile unique bir sayı elde edip, index olarak kullanıyoruz.
            return &x[((long)order_id<<40) + ((int)book_idx << 8) + side];
        }
};
...
class order_book
{
 public:
    ...
    static int BOOK;
  int x;
  static constexpr size_t MAX_BOOKS = 1<<24;
  ...
  using sorted_levels_t = std::vector<price_level>; //BURAYI AĞAÇ YAPICAZ
...
/*
 * Function to print the wanted order book every time
 * an order associated with the order book takes place.
 */


  void print_book(int ID,order_t *cur)
    {
        if((size_t )cur->book_idx == (size_t) ID)
        {
            int counter_1 = 0;
            int counter_2 = 0;
            std::cout << "#### PRINTING ORDER BOOK HAS STARTED ####" << std::endl;
            std::cout << "bids for order book " << ID << " : " << std::endl;
            if(order_book::s_books[(size_t) ID].m_bids.size() == 0)
            {
            ...
            for(int i=order_book::s_books[(size_t) ID].m_bids.size()-1;i>=0;i--)
            {
                std::cout << counter_1+1 << " - " << (int) order_book::s_books[(size_t) ID].m_bids.at(i).m_price;
                std::cout << "\t";
                std::cout << (int) order_book::s_books[(size_t) ID].s_levels[order_book::s_books[(size_t) ID].m_bids.at(i).m_ptr].m_qty << std::endl;
                counter_1++;
            }

            std::cout << "-------------------------------------------------------------" << std::endl;

            std::cout << "asks for order book " << ID << " : " << std::endl;
            if(order_book::s_books[(size_t) ID].m_offers.size() == 0)
            {
                std::cout << "No entries found." << std::endl;
            } else
            {
            ...
            for(int i=order_book::s_books[(size_t) ID].m_offers.size()-1;i>=0;i--)
            {
                std::cout << counter_2+1 << " - " << -1 * (int) order_book::s_books[(size_t) ID].m_offers.at(i).m_price;
                std::cout << "\t";
                std::cout << (int) order_book::s_books[(size_t) ID].s_levels[order_book::s_books[(size_t) ID].m_offers.at(i).m_ptr].m_qty << std::endl;

                counter_2++;

            }

            std::cout << "#### PRINTING ORDER BOOK HAS ENDED ####" << std::endl;
            std::cout << std::endl;
  ...
  static void add_order(order_id_t const oid, book_id_t const book_idx,
                        sprice_t const price, qty_t const qty,int side_)
  {
#if TRACE
    printf("ADD %lu, %u, %d, %u", oid, book_idx, price, qty);
#endif  // TRACE
    order *order = oid_map.get(oid,book_idx,side_);
    order->m_qty = qty;
  ...
  void ADD_ORDER(order_t *order, sprice_t const price, qty_t const qty)
  {
    ...
    s_levels[order->level_idx].m_qty = s_levels[order->level_idx].m_qty + order->m_qty;

      print_book(BOOK,order);

  }
  static void delete_order(order_id_t const oid,book_id_t const book_idx,int side_)
  {
#if TRACE
    printf("DELETE %lu\n", oid);
#endif  // TRACE
    order_t *order = oid_map.get(oid,book_idx,side_);
    s_books[size_t(order->book_idx)].DELETE_ORDER(order);



  }
  static void cancel_order(order_id_t const oid, qty_t const qty,book_id_t const book_idx,int side_)
  {
/*
 * Cancel order message is not generated by BIST.
 */
  }
  ...
  static void execute_order(order_id_t const oid, qty_t const qty,book_id_t const book_idx,int side_)
  {
#if TRACE
    printf("EXECUTE %lu %u\n", oid, qty);
#endif  // TRACE
